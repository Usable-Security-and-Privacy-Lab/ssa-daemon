...
int client_session_resumption_enabled(SSL_CTX* ctx) {
    ...
    return (cache_mode & SSL_SESS_CACHE_CLIENT) ? 1 : 0;
}
...
int has_session_cache(SSL_CTX* ctx) {
    ...
    hsmap_t* cache = SSL_CTX_get_ex_data(ctx, SESS_CACHE_INDEX);
...
int session_cache_new(SSL_CTX* ctx) {
    ...
    hsmap_t* session_cache = NULL;
    int* cache_reference_cnt = NULL;
    int ret;

    session_cache = str_hashmap_create(SESSION_CACHE_NUM_BUCKETS);
...
err:
    if (session_cache != NULL)
        str_hashmap_free(session_cache);
...
void session_cache_free(SSL_CTX* ctx) {
    ...
    hsmap_t* session_cache = SSL_CTX_get_ex_data(ctx, SESS_CACHE_INDEX);
    ...
    if (*cache_ref_cnt > 1) {
        *cache_ref_cnt -= 1;
        ...
        free(cache_ref_cnt);
        str_hashmap_deep_free(session_cache, free_ssl_session);
    }
...
int session_resumption_setup(SSL* ssl, char* hostname_port) {
    ...
    session = str_hashmap_get(session_cache, hostname_port);
    if (session == NULL)
        return 0;
    ...
    str_hashmap_del(session_cache, hostname_port);
 ...
 * ID during a connection; the session is cached within an internal opaque cache
 * in addition to the caching done here.
...
int new_session_cb(SSL* ssl, SSL_SESSION* session) {
    ...
    ret = SSL_SESSION_set_ex_data(session, HOSTNAME_PORT_INDEX, host_port);
    if (ret != 1)
        goto err;
 ...
 * holds. This callback effectively allows the hsmap_t* session cache to stay
...
void remove_session_cb(SSL_CTX *ctx, SSL_SESSION *session) {
    ...
    host_port = SSL_SESSION_get_ex_data(session, HOSTNAME_PORT_INDEX);
    if (host_port == NULL)
        return;
