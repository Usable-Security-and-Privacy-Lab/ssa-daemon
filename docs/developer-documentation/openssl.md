...
* To compile, two flags are needed: `-lssl` and `-lcrypto`. So, using gcc, a simple program using OpenSSL functions would compile when using: `gcc -o my_program my_program.c -lssl -lcrypto`. The flags must go in this order, as the library `-lssl` relies on the library `-lcrypto` to compile.
...
* To create a new context, use the function `SSL_CTX_new(SSL_METHOD *method)`. It returns a pointer to a new SSL_CTX struct.
...
* Before you can verify a connection, you have to set the location of accepted Certificate Authority certificates (CAs). These are stored on the computer; usually for Linux systems such as Ubuntu this is at `/etc/ssl/certs`. For Fedora, Certificate Authorities are actually all stored in a file located at TODO: put file here.
...
* To set a standard security level, use `SSL_CTX_set_security_level(int level)`. Its `level` parameter defaults to 1 when not set.
    ...
    * Level 3: 128 bits (such as SHA-256), forward secrecy mandatory, only TLS 1.1+; session tickets also disabled
...
* For the TLS 1.3 protocol we use the function `SSL_CTX_set_ciphersuites(SSL_CTX *ctx, char *str)`. Where the string contains some combination of the five accepted ciphers. The accepted ciphers are:
    ...
    * Note that the ciphers should be placed in the order of preference from first to last, using a colon (‘:’) to delimit each cipher name.
...
* To set an End Entity certificate that is stored in a file, use `SSL_CTX_use_certificate_file(SSL_CTX *ctx, char *file, int type)`. `file` is a string representing the path to the certificate, and `type` is one of the following constants (depending on the way it’s encoded): `SSL_FILETYPE_PEM` or `SSL_FILETYPE_ASN1`. If multiple certificates are stored in the file (only possible in PEM format), then only the first certificate will be selected.
...
* To build the certificate chain, use `SSL_CTX_build_cert_chain(SSL_CTX *ctx, long flags)` with the following flag: `SSL_BUILD_CHAIN_FLAG_CHECK`.
