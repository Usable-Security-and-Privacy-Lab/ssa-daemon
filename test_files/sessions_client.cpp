...
const char* HOSTNAME  = "localhost"; /* Does sessions */
const char* PORT = "443";
...
const char* ERR_SOURCE_DISABLE_SESS = "TLS_SESSION_REUSE setsockopt()";
...
public:
    ...
    ClientSessionTests() {
        ...
        context_fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TLS);
        if (context_fd < 0) {
        ...
        if (hostname_ret != 0) {
            print_socket_error(context_fd, ERR_SOURCE_SET_HOSTNAME);
            goto err;
        }
...
TEST_F(ClientSessionTests, TenSocketSessionReuse) {
    ...
    for (int i = 0; i < TEN_FDS; i++) {
        ...
        /*
        int send_ret = send(fds[i], "GET / HTTP/1.1\r\n\r\n", strlen("GET / HTTP/1.1\r\n\r\n"), 0);
        */

        /*
        char buf[1000];
        int recv_ret = recv(fds[i], buf, 1000, 0);
        */
...
TEST_F(ClientSessionTests, TwoSocketDisabledSessionReuse) {
    ...
    for (int i = 0; i < TWO_FDS; i++) {
        ...
        int disable_session_ret = setsockopt(fds[i], IPPROTO_TLS,
                    TLS_SESSION_REUSE, &reuse_sessions, sizeof(reuse_sessions));
        int disable_session_errno = errno;

        if (disable_session_ret != 0)
            print_socket_error(fds[i], ERR_SOURCE_DISABLE_SESS);

        EXPECT_EQ(disable_session_errno, 0);
        ASSERT_EQ(disable_session_ret, 0);
        ...
        /* we want sessions not to be reused here */
        ASSERT_EQ(session_reused, 0);
    }
